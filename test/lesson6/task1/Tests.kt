...
class Tests {
    ...
    @Test
    fun lab2FromRoman() {
        //Т.к. M это максимальная римская цифра, то она может повторятся больше 3х раз, таким образом задание решено не верно
        //Посмотрим пример в котором ответ 7000, но получаем -1 т.к. MMMMMMMM > 3 повторений 1го символа.
        assertEquals(-1, fromRoman("MMMMMM"))

        //Тут -1 т.к. I может идти только 3 раза подряд
        assertEquals(-1, fromRoman("IIIIIIIIIII"))

        //Посмотрим как поведет себя с ракеткой)
        assertEquals(-1, fromRoman("🚀"))

        //Ну и пустой тоже проверим
        assertEquals(-1, fromRoman(""))
